...
public class ProductImagesController {
    ...
    @PreAuthorize("hasRole('PRODUCTS')")
    @RequestMapping(value = "/admin/products/product/image/imageInsert.html", method = RequestMethod.GET)
    public void imageInsert(@RequestParam("startId") long startId,
        ...
        for (long i = startId; i <= endId; i++) {
            ...
            ProductImage productImage = new ProductImage();
            String imagePath = sb.append(productSku).append(".jpg").toString();
            MultipartFile image = null;
            try {
                image = stream2Multi(imagePath);
                productImage.setImage(image.getInputStream());
                productImage.setProductImage(image.getOriginalFilename());
            } catch (Exception e) {

            }

            productImage.setImage(new FileInputStream(new File(imagePath)));
            productImage.setProductImage(productSku + ".jpg");
            productImage.setProduct(product);
            productImage.setDefaultImage(true);//default image is uploaded in the product details
            contentImagesList.add(productImage);
            product.setImages(images);
            productImageService.addProductImages(product, contentImagesList);
            sb.replace(0,sb.length(),filePath);
        }
    }


    private MultipartFile stream2Multi(String pathname) throws Exception {


        // 读入 文件
        File file = new File(pathname);
        FileInputStream in_file = new FileInputStream(file);
//        MultipartFile multi = new MockMultipartFile(pathname, in_file);
